        ...
        <div class="col-12">
            <div class="card card-outline card-warning">
                <div class="card-header">
                    <h3 class="card-title">{{__('slides')}} ({{count($records)}})</h3>
                    <div class="card-tools">
                        <a class="btn btn-sm btn-warning" href="{{route('slides.create')}}">
                                ...
                                @if(count($records) > 0)
                                    @foreach($records as $index=>$record)
                                        <tr>
                                            ...
                                            <td>
                                                <a class="btn btn-info btn-sm" href="{{route('slides.edit', $record->id)}}">
                                                ...
                                                <a class="btn btn-sm btn-danger" href="#" onclick="event.preventDefault(); document.getElementById('delete-form-{{$record->id}}').submit();">
                                                    ...
                                                    <form id="delete-form-{{$record->id}}" action="{{ route('slides.destroy',$record->id) }}" method="POST" class="d-none">
                                                        @csrf
                                                        @method('delete')
                                                    </form>
                                                </a>
